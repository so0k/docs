...
<Tip>Try the [Deploy Applications with CDK Terrain](/terraform/tutorials/cdktn/cdktn-applications?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.</Tip>
...
<Tip>Try the [Deploy Multiple Lambda Functions with TypeScript](/terraform/tutorials/cdktn/cdktn-assets-stacks-lambda) tutorial. This tutorial guides you through a multi-stack application.</Tip>
...
Refer to [Best Practices](/docs/create-and-deploy/best-practices#separate-business-units-with-stacks) for more details about when to create multiple stacks and how to structure them.
...
If you want to keep the result of the function attached to one stack and save its state, create a [Terraform Local value](/docs/concepts/variables-and-outputs#local-values) and expose it. The following example creates a `TerraformLocal`.
