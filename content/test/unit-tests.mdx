...
- **C#:** Follow the [Getting Started with xUnit.net guide](https://xunit.net/docs/getting-started/netcore/cmdline). The assertions for CDKTN are available in the `cdktn` package under `Testing`.
...
You can produce invalid Terraform configuration if you are using escape hatches in your CDK Terrain application. You may use an escape hatch when setting up a [remote backend](/docs/concepts/remote-backends) or when [overriding resource attributes](/docs/concepts/resources#escape-hatch)
