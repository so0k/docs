...
The `cdktf.json` file is where you can supply custom configuration settings for your application and define the [providers](/docs/concepts/providers) and [modules](/docs/concepts/modules) that you want to use. When you initialize a new CDK Terrain project with a [built-in template](/docs/create-and-deploy/project-setup), the template generates a basic `cdktf.json` file in your root directory that you can customize for your application. Refer to the [Project Setup documentation](/docs/create-and-deploy/project-setup) for more information about initializing a new project.
...
The most basic configuration only defines `app`. This is useful when you plan to use [pre-built providers](/docs/concepts/providers#install-pre-built-providers) and you don't need to generate any provider or module bindings.
...
You can enable or disable crash reporting by setting the `sendCrashReports` property to `true` or `false`. Sending crash reports helps our team improve the CLI faster. Refer to [Telemetry](/docs/telemetry#crash-reporting) for more information about what we track.
